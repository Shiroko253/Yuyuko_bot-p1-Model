...
class FishingButtons(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        if self.original_message:
            try: await self.original_message.edit(content="釣魚操作已超時...", view=self)
            except: pass
    ...
    async def save_fish(self, button, interaction):
        if self.data_manager.is_backing_up:
            ...
            return
        try:
            button.disabled = True; button.label = "封存中..."
            await interaction.response.edit_message(view=self)

            user_id, guild_id = str(interaction.user.id), str(interaction.guild.id) if interaction.guild else "DM"
            fish_record = {"name": self.latest_fish_data["name"], "rarity": self.latest_fish_data["rarity"], "size": self.latest_fish_data["size"], "rod": self.current_rod, "caught_at": datetime.now(self.cog.TIMEZONE).isoformat()}
            ...
            button.label = "✅ 已封存"; button.style = discord.ButtonStyle.success
            await interaction.edit_original_response(view=self)
        except Exception as e:
            logger.error(f"保存漁獲失敗: {e}")
...
class Fish(commands.Cog):
    ...
    def calculate_rarity_weights(self, fish_data: list) -> dict:
        if self.rarity_weights_cache: return self.rarity_weights_cache
    ...
    def generate_fish_data(self, fish_data: list, main_job: str = "無職業", sub_job: str = "無副職") -> dict:
        if not fish_data: return {"name": "神秘櫻花魚", "rarity": "common", "size": 0.5}
        ...
        rarity_pools = {}
        for fish in fish_data: rarity_pools.setdefault(fish.get("rarity", "common").lower(), []).append(fish)
        ...
        except: fish_size = 0.5

        return {"name": selected_fish.get("name", "未知"), "rarity": selected_fish.get("rarity", "common").lower(), "size": fish_size, "is_empty_handed": False}
    ...
    def create_fishing_embed(self, fish_data: dict, current_rod: str, main_job: str, sub_job: str) -> discord.Embed:
        ...
        # 決定情境描述
        if main_job == "漁夫": scenario = FISHING_SCENARIOS["漁夫"]
        elif sub_job == "釣魚佬": scenario = FISHING_SCENARIOS["釣魚佬"]
        else: scenario = FISHING_SCENARIOS["普通"]
        ...
        embed = discord.Embed(
            ...
            color=info["color"], timestamp=datetime.now(self.TIMEZONE)
        )
        ...
        size = fish_data['size']
        if size >= 10: comment = "天啊！這是巨物級別的漁獲！"
        elif size >= 5: comment = "好大的一條魚～幽幽子都驚訝了！"
        elif size >= 2: comment = "不錯的收穫呢！"
        else: comment = "小小的也很可愛～"
    ...
    async def fish(self, ctx: ApplicationContext):
        ...
        if not fish_data:
            await ctx.respond("幽幽子迷糊了，無法讀取魚資料～", ephemeral=True)
            return
        ...
        if latest_fish_data.get("is_empty_handed"):
            embed = discord.Embed(
                ...
                color=discord.Color.light_gray(), timestamp=datetime.now(self.TIMEZONE)
            )
        else:
