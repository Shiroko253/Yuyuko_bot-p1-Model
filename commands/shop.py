...
class ShopPagesView(View):
    ...
    def __init__(self, ctx: discord.ApplicationContext, shop_items: list, total_pages: int, data_manager, cog: Shop):
    ...
    def update_buttons(self):
        ...
        if self.current_page > 1:
            prev_button = Button(label="上一頁", style=discord.ButtonStyle.secondary, emoji="⬅️", row=0)
            prev_button.callback = self.prev_page
            self.add_item(prev_button)
        
        if self.current_page < self.total_pages:
            next_button = Button(label="下一頁", style=discord.ButtonStyle.secondary, emoji="➡️", row=0)
            next_button.callback = self.next_page
            self.add_item(next_button)
        
        buy_button = Button(label="選購", style=discord.ButtonStyle.success, emoji="🛒", row=1)
        buy_button.callback = self.start_buy
        self.add_item(buy_button)
        
        close_button = Button(label="關閉商店", style=discord.ButtonStyle.danger, emoji="❌", row=1)
    ...
    async def prev_page(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != self.ctx.author.id:
            await interaction.response.send_message("呀啦呀啦～這不是你的商店頁面哦!", ephemeral=True)
            return
    ...
    async def close_shop(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(
            title="🌸 商店已關閉",
            description="呼呼～感謝光臨幽幽子的商店!\n櫻花樹下歡迎你隨時再來～",
            color=SHOP_COLOR
        )
        embed.set_footer(text="期待下次見面 · 幽幽子", icon_url=self.cog.bot.user.avatar.url if self.cog.bot.user.avatar else None)
...
class BuyModal(Modal):
    ...
    def __init__(self, ctx: discord.ApplicationContext, shop_items: list, data_manager, cog: Shop):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            view = ConfirmBuyView(self.ctx, item, quantity, total_price, self.data_manager, self.cog)
            ...
            try:
                await interaction.response.send_message("❌ 購買流程發生錯誤,請稍後再試!", ephemeral=True)
            except:
                await interaction.followup.send("❌ 購買流程發生錯誤,請稍後再試!", ephemeral=True)


class ConfirmBuyView(View):
    """購買確認視圖"""
    
    def __init__(self, ctx: discord.ApplicationContext, item: dict, quantity: int, total_price: float, data_manager, cog: Shop):
    ...
    async def confirm(self, button: Button, interaction: discord.Interaction):
        """確認購買"""
        if interaction.user.id != self.ctx.author.id:
            await interaction.response.send_message("呀啦呀啦～這不是你的購買確認哦!", ephemeral=True)
            return
        ...
        try:
            ...
            logger.info(f"💰 {interaction.user.name} 購買了 {self.quantity} 個 {self.item.get('name')}, 花費 {self.total_price:.2f} 幽靈幣")
            
        except Exception as e:
            logger.error(f"❌ 購買執行失敗: {e}", exc_info=True)
            await interaction.followup.send("❌ 購買執行失敗,請稍後再試!", ephemeral=True)
