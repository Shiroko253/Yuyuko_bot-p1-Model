...
class QuizView(discord.ui.View):
    ...
    async def on_timeout(self):
        ...
        try:
            ...
            logging.warning(f"QuizView on_timeout 編輯訊息失敗: {e}")
...
class QuizCog(commands.Cog):
    ...
    async def quiz(self, ctx: discord.ApplicationContext):
        ...
        try:
            ...
            logging.error(f"讀取 quiz.json 失敗: {e}")
            return await ctx.respond("❌ 題庫讀取失敗，請聯絡管理員！")
        if not quiz_data:
        ...
        if not question or not correct_answer or len(incorrect_answers) != 3:
            logging.warning(f"quiz.json 題目格式錯誤: {question_data}")
        ...
        try:
            ...
            logging.warning(f"QuizCog quiz 設定 view.message 出錯: {e}")
