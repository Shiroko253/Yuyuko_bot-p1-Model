    ...
    async def hit(self, button: discord.ui.Button, interaction: discord.Interaction):
        try:
            ...
            if player_total > 21:
                ...
                ).set_footer(text="遊戲結束"), view=None)
                return
            ...
            ).set_footer(text="可選擇繼續抽牌或停牌"), view=self)
        except Exception as e:
        ...
        try:
            ...
            if game_data["double_down_used"]:
                await interaction.edit_original_response(embed=discord.Embed(
                    ...
                    color=discord.Color.red()
                ).set_footer(text="每局只能使用一次雙倍下注"), view=self)
                return
            ...
            if user_balance < bet:
                await interaction.edit_original_response(embed=discord.Embed(
                    title="🌸 嘻嘻，靈魂不夠喲～ 🌸",
                    description=f"你的幽靈幣只有 {user_balance:.2f}，不足以雙倍下注 {doubled_bet:.2f} 哦～",
                    color=discord.Color.red()
                ).set_footer(text="請先補充你的幽靈幣"), view=self)
                return
            ...
            if player_total > 21:
                ...
                for child in self.children:
                    child.disabled = True
                await interaction.edit_original_response(embed=embed.set_footer(text="遊戲結束"), view=None)
                return
    ...
    async def blackjack(self, ctx: discord.ApplicationContext, bet: float):
        try:
            ...
            await ctx.respond(embed=discord.Embed(
                ...
                color=discord.Color.red()
            ).set_footer(text="如有問題請聯絡管理員"), ephemeral=True)
