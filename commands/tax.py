    ...
    async def tax(self, ctx: discord.ApplicationContext):
        ...
        if not ctx.author.guild_permissions.administrator:
            await ctx.respond(
                embed=discord.Embed(
                    title="🌸 權限不足！🌸",
                    description="只有管理員才能徵稅哦～幽幽子可不想被亂收稅！",
        ...
        if not balance.get(guild_id):
            await ctx.followup.send(embed=discord.Embed(
                title="🌸 無人可稅！🌸",
                description="這個伺服器還沒有人有幽靈幣哦～快去玩遊戲賺錢吧，幽幽子在等美味的供品～",
                color=discord.Color.red()
            ))
            return
        ...
        if not taxed_users:
            await ctx.followup.send(embed=discord.Embed(
                title="🌸 無人可稅！🌸",
                description="沒有人有足夠的幽靈幣可以徵稅哦～幽幽子只好餓肚子啦！",
                color=discord.Color.red()
            ))
            return
        ...
        embed = discord.Embed(
            title="🌸 幽幽子的動態稅金徵收！🌸",
            description=(
                f"幽幽子在櫻花樹下對伺服器內所有亡魂徵收美味稅金，國庫又豐盈啦！\n"
                f"徵稅執行者：**{executor}**\n\n"
                f"被徵稅者：\n" + "\n".join(taxed_users) + f"\n\n"
        ...
        embed.set_footer(text="幽幽子：賞花、吃點心、收稅金三連發！")
